import { Search, Filter, Plus, Edit, Trash2, Clock, Download } from 'lucide-react';

export default function InventariTab({ 
  role, loading, ordinadors, searchTerm, setSearchTerm, searchField, setSearchField,
  selectedIds, setSelectedIds, setIsBulkModalOpen, handleBulkDelete, obrirModalNou, obrirModalEdicio, handleEsborrarPortatil, formatData, getBadgeColor
}) {
  ...
  const exportarA_CSV = () => {
    ...
    // Convertimos cada fila filtrada a texto CSV
    const rows = ordinadors.map(ord => [
      ...
      `"${ord.nom_alumne || ''}"`,
      `"${ord.classe_alumne || ''}"`,
      `"${(ord.observacions || '').replace(/"/g, '""')}"`, // Escapar comillas si hay
      `"${formatData(ord.data_modificacio)}"`
    ]);

    // Unimos cabeceras y filas con salto de línea (añadiendo el BOM de Excel para que reconozca los acentos correctamente)
    const csvContent = "\uFEFF" + [headers.join(","), ...rows.map(e => e.join(","))].join("\n");
    
    // Creamos el archivo descargable invisible
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-4">
        ...
        <div className="flex gap-3 w-full xl:w-auto justify-end items-center">
          <button 
            ...
            title="Exporta la llista filtrada actual a Excel/CSV"
          >
              ...
              {role === 'admin' && (
                <th className="p-4 w-12 text-center">
                  <input type="checkbox" className="w-4 h-4 rounded text-blue-600 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 cursor-pointer" 
                    ...
                    checked={selectedIds.length === ordinadors.length && ordinadors.length > 0} 
                  />
                </th>
              )}
